                ...
                <div className="max-w-xl">
                    <div className="mb-6">
                        <div className="flex items-center gap-5">
                            ...
                            {/* Trophy icon */}
                            <div
                                className="w-20 h-20 bg-black/80 rounded-2xl p-4 backdrop-blur-xl
                                  border border-orange-500/30 shadow-[0_0_30px_rgba(74,222,128,0.3)]
                        ...
                        <p className="text-gray-300 text-base mt-4">Celebrating our achievements in the Web3 and cryptocurrency space in New Zealand.</p>
                    </div>

                    {/* Current award details with animation */}
                    <div className="mb-8 relative h-[220px] overflow-hidden">
                            ...
                            {awards.map((award, index: number) => (
                                <div key={index} style={{ position: 'absolute', top: 0, left: 0, right: 0, opacity: index === currentImageIndex ? 1 : 0, transform: `translateY(${index === currentImageIndex ? 0 : 30}px)`, transition: 'opacity 0.5s ease, transform 0.5s ease', pointerEvents: index === currentImageIndex ? 'auto' : 'none' }}>
                                    <div className="mb-4 pb-4 border-b border-orange-500/30">
                                        <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">{award.title}</h3>
                    ...
                    {/* Award navigation dots */}
                    <div className="flex items-center gap-3 mb-4">
                        <div className="flex items-center gap-2">
                            {awards.map((_, index: number) => (
                                <button key={index} onClick={() => setCurrentImageIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentImageIndex ? 'bg-orange-500 w-8' : 'bg-gray-500 hover:bg-gray-400'}`} aria-label={`View award ${index + 1}`} />
                            ))}
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-5">
                        <div className="text-center p-3 bg-black/40 backdrop-blur-sm rounded-lg border border-orange-500/30">
                            <p className="text-xl font-bold text-orange-400">{awards.length}</p>
                            <p className="text-xs text-gray-300">Awards Won</p>
                        </div>
                        <div className="text-center p-3 bg-black/40 backdrop-blur-sm rounded-lg border border-orange-500/30">
                            <p className="text-xl font-bold text-orange-400">2024-25</p>
                            <p className="text-xs text-gray-300">Award Period</p>
                        </div>
                        <div className="text-center p-3 bg-black/40 backdrop-blur-sm rounded-lg border border-orange-500/30">
                            <p className="text-xl font-bold text-orange-400">Web3</p>
